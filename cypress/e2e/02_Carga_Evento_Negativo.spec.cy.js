describe('Carga de Evento - Casos Negativos Completos', () => {
  ...
  // CASO NEGATIVO 2: Tipo de entrada no seleccionado
 
  it('Caso negativo 2: tipo de entrada no seleccionado', () => {
    cy.contains('a', 'Cargar Evento').click();
    cy.get('[data-cy="titulo-form"]').should('contain.text', 'Cargar Función');
    ...
    cy.get('div[role="dialog"]').contains('span', '20').click({ force: true });
    cy.contains('button', 'Siguiente').click({ force: true });
    cy.contains('Debe seleccionar al menos un tipo de entrada').should('be.visible');
  });
// CASO NEGATIVO 3: Lugar del evento vacío
it('Caso negativo 3: lugar del evento vacío', () => {
  cy.contains('a', 'Cargar Evento').click();
  cy.get('[data-cy="titulo-form"]').should('contain.text', 'Cargar Función');
  ...
  cy.get('div[role="dialog"]').contains('span', '20').click({ force: true });
  cy.contains('button', 'Siguiente').click({ force: true });
  cy.contains('El lugar del evento es obligatorio').should('be.visible');
});
